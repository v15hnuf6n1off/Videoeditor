...
async def encode(event, msg, scale=0):
    ...
    if scale == 240:
        ...
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -c:v libx264 -pix_fmt yuv420p -preset faster -s 1920x1080 -crf 24 -c:a libopus -ac 2 -ab 128k -c:s copy """{out}""" -y'
    try:
